...
def _fixture(
    ...
    *,
    ...
    ui_defect_bucket_missing_post_rerun_evidence: str = "",
    ...
    gap_checks = [
        {"key": "raw_issue_schema_report_present", "pass": True},
        {"key": "raw_issue_schema_pass", "pass": raw_issue_schema_pass},
        {"key": "normalized_issue_schema_report_present", "pass": True},
        {"key": "normalized_issue_schema_pass", "pass": normalized_issue_schema_pass},
        {"key": "final_visual_audit_report_present", "pass": final_artifacts},
        {"key": "visual_audit_index_present", "pass": True},
        {"key": "visual_audit_full_scope_allowed", "pass": gap_pass},
    ]
    gap_check_passes = [bool(item["pass"]) for item in gap_checks]
    if visual_audit_schema_gap_check_mismatch:
        gap_checks[-1] = {**gap_checks[-1], "pass": not gap_checks[-1]["pass"]}
    gap_counter_payload = (
        {
    ...
    gap_backfill_overlay_source_path = (
    ...
    repair_raw_count = 0 if raw_issue_schema_pass else 7
    if visual_audit_repair_plan_count_mismatch:
        repair_raw_count += 1
    ...
    for key in defect_bucket_keys:
        ...
        if ui_defect_bucket_missing_post_rerun_evidence:
            item["post_rerun_required_evidence"] = [
                value
                for value in item["post_rerun_required_evidence"]
                if value != ui_defect_bucket_missing_post_rerun_evidence
            ]
            ...
            root / "visual_audit_schema_gap_v4_4.json",
            {
                ...
                **gap_counter_payload,
                ...
                "raw_issue_backfill_overlay_summary": {
                    "path": str(backfill_overlay_path),
                    "exists": True,
                    "generated_at": backfill_overlay_generated_at,
                ...
                "raw_issue_repair_plan_present": True,
                ...
                "raw_issue_repair_plan_summary": {
                    ...
                    "raw_noncompliant_issue_count": repair_raw_count,
                    "missing_replacement_count": 0,
                    "lossy_normalized_issue_count": 0 if raw_issue_schema_pass else 5,
                    "historical_artifacts_modified": False,
                    "normalized_cannot_replace_raw": True,
                },
...
def test_product_evidence_gate_blocks_locked_006_when_bucket_closure_lacks_reference_compare_evidence() -> None:
    with TemporaryDirectory() as tmp:
        result = _build(
            _fixture(Path(tmp), ui_defect_bucket_missing_post_rerun_evidence="reference_compare_v4")
        )
...
def test_product_evidence_gate_blocks_release_when_visual_audit_schema_gap_checks_do_not_match_counters() -> None:
    with TemporaryDirectory() as tmp:
        result = _build(_fixture(Path(tmp), visual_audit_schema_gap_check_mismatch=True))
        ...
        counter_contract = check["details"]["visual_audit_schema_gap"]["counter_contract"]
        assert counter_contract["pass"] is False
        assert counter_contract["checks_count_matches"] is True
        assert counter_contract["check_pass_counts_match"] is False
...
def test_product_evidence_gate_blocks_release_when_backfill_overlay_source_is_stale() -> None:
    with TemporaryDirectory() as tmp:
        result = _build(_fixture(Path(tmp), visual_audit_backfill_overlay_source_mismatch=True))
...
def test_product_evidence_gate_blocks_release_when_backfill_overlay_is_older_than_raw_report() -> None:
    with TemporaryDirectory() as tmp:
        result = _build(_fixture(Path(tmp), visual_audit_backfill_overlay_stale_generated_at=True))
        ...
        assert contract["overlay_generated_at_not_older_than_raw"] is False
...
def test_product_evidence_gate_blocks_release_when_repair_plan_count_disagrees_with_raw_report() -> None:
    with TemporaryDirectory() as tmp:
        result = _build(_fixture(Path(tmp), visual_audit_repair_plan_count_mismatch=True))
        ...
        contract = check["details"]["visual_audit_schema_gap"]["repair_plan_contract"]
        assert contract["pass"] is False
        assert contract["repair_raw_count_matches_raw"] is False
