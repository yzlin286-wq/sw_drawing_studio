...
- `tools/validation/product_evidence_gate_v4_4.py`: now includes `visual_audit_schema_proof_pass`. The gate requires final `drw_output/visual_audit_report_v3_0.xlsx`, raw `drw_output/issue_schema_validation.json` with `pass=true` and `noncompliant_issue_count=0`, and normalized `drw_output/issue_schema_validation_normalized.json` with `pass=true`; normalized proof is explicitly supporting-only and cannot replace raw historical issue compliance.
