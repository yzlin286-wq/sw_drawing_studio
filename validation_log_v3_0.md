...
- `test_v4_4_product_evidence_gate.py`: verifies a complete synthetic evidence fixture can pass, readiness blocks all real-CAD follow-on actions, 006 UI acceptance failure blocks expansion, 006 UI PASS can allow only the ref6 expansion start while keeping `LB26001_36/full_129` blocked until all six pass, missing final artifacts block release, raw issue schema failure blocks release even when normalized schema passes, and the gate remains file-only.
