...
- `drw_output/diagnostics/unguarded_solidworks_entrypoints.json`: refreshed with schema `sw_drawing_studio.unguarded_solidworks_entrypoints.v4_4`; current status is `pass`, `entrypoint_count=522`, `unguarded_or_unknown_count=0`, `ui_thread_direct_risk_count=0`, `service_direct_risk_count=0`, `system_health_ui_thread_direct_probe_count=0`, `worker_backed_model_client_count=1`, `legacy_service_adapter_count=1`, `background_watchdog_probe_count=1`, `validation_tool_requires_manual_lock_count=47`, `external_addin_needs_host_lock_count=20`, and `external_addin_host_lock_contract_status=pass`.
...
- `drw_output/diagnostics/solidworks_stability_gate_v4_4.json`: summary is now `status=pass`, `pass=true`, `release_ready=false`, with no warning reasons. This proves the SolidWorks Stability Gate only; it does not make the product release-ready.
...
- `app/workers/llm_action_worker.py` and `app/ui/settings_dialog.py`: Settings dialog model connection testing now runs through `JobRuntimeFacade.start_llm_action(action="test_connection")`, avoiding UI-thread network retry sleeps.
...
- 2026-06-26 Stability Gate bottom-level evidence is now directly consumed by the Product Evidence Gate. Refreshed `drw_output/diagnostics/product_evidence_gate_v4_4.json` records `source_artifacts.entrypoint_report`, `source_artifacts.lock_test_report`, and `source_artifacts.conflict_report`; checks `solidworks_entrypoint_scan_report_pass`, `solidworks_lock_test_report_pass`, and `solidworks_conflict_report_ok` are all `pass`. The underlying reports were refreshed by `test_v4_4_solidworks_stability_gate.py`: entrypoint scan has `entrypoint_count=522`, zero unguarded/UI/service/System Health direct-risk buckets, and `external_addin_host_lock_contract_status=pass`; lock test passes all six checks; conflict report is `level=OK` with zero conflict counts.
- 2026-06-26 UI heavy-work scanner gate added. `app/services/solidworks_entrypoint_scanner.py` now detects UI-scope direct OCR/YOLO/Vision QC/visual-audit batch/staged CAD/batch-validation imports or calls and reports `ui_thread_heavy_work_count` plus `ui_thread_heavy_work_calls`. The Stability Gate and Product Evidence Gate both require this count to remain zero, in addition to the existing UI subprocess, QThreadPool, COM/probe, and service-direct risk buckets. Current refreshed entrypoint evidence is `status=pass`, `entrypoint_count=527`, `unguarded_or_unknown_count=0`, `ui_thread_direct_risk_count=0`, `ui_thread_subprocess_call_count=0`, `ui_thread_heavy_work_count=0`, `ui_threadpool_worker_count=0`, `service_direct_risk_count=0`, and `external_addin_host_lock_contract_status=pass`.
- 2026-06-26 latest no-CAD refresh remains blocked. No SolidWorks document operation, CAD generation, OCR, YOLO, visual-audit batch, staged CAD validation, or batch validation was run in this slice. `solidworks_lock_test_result.json` passes all six lock checks, but `solidworks_stability_gate_v4_4.json` is `status=warning`, `pass=false` because `conflict_report.json` is `level=WARNING` with one SolidWorks process and no worker lock. `lb26001_006_regression_readiness_v4_2.json` is blocked by `solidworks_unsaved_document_visible`, `lb26001_006_rerun_packet_v4_2.json` stays `packet_build_ready=true` and `real_cad_allowed_now=false`, and `product_evidence_gate_v4_4.json` is `status=blocked_by_solidworks_stability_gate`. The 006 locked CAD rerun and the requested `007/008/009/015/022` expansion remain forbidden until 006 has fresh locked CAD evidence plus application Drawing Review UI screenshot PASS.
- 2026-06-26 canonical UI screenshot proof tightened. `tools/validation/product_evidence_gate_v4_4.py` now requires the canonical 006 application UI screenshot to be a decodable image with minimum dimensions `1000x600`, and it requires the bound UI report/manual screenshot checks, v6 visual acceptance, and reference-compare-with-UI checks to pass before `canonical_006_ui_visual_review_pass` can pass. Refreshed `drw_output/diagnostics/product_evidence_gate_v4_4.json` records the current canonical screenshot file as present, nonempty, decodable, and `2600x1400`, so the evidence file is valid; the gate still fails because the visual review itself remains `need_review` with `vision_qc_v6_visual_acceptance_pass=false` and `reference_compare_v4_pass=false`. This reinforces that screenshot evidence is required but not automatically accepted.
- 2026-06-26 canonical UI bucket-closure proof tightened. `tools/validation/apply_ui_visual_review_v4.py` now reads the 006 defect-bucket contract and writes `ui_defect_bucket_closure` into canonical `ui_visual_review.json`; a canonical 006 UI review cannot pass unless every required bucket is visually closed by manual/application-UI judgement. `tools/validation/product_evidence_gate_v4_4.py` now requires `checks.ui_defect_bucket_closure_pass=true` for `canonical_006_ui_visual_review_pass`, and `tools/validation/lb26001_006_rerun_packet_v4_2.py` source-guards this behavior. Refreshed current UI evidence remains `need_review` with blocking key `ui_defect_bucket_closure_not_proven`; missing closure buckets are `dimension_visual_overdense`, `dimension_lane_wrong`, `note_missing_or_wrong`, `titlebar_incomplete`, `projection_view_style_mismatch`, and `callout_missing`. No CAD, COM document open, OCR, YOLO, visual-audit batch, staged CAD validation, or batch validation was run.
- 2026-06-26 requested six-drawing completion gate tightened. `tools/validation/lb26001_requested_drawings_status_v4_2.py` now emits per-drawing required artifact evidence for `drawing_blueprint`, `dimension_validation`, `reference_compare`, `vision_qc`, and `ui_visual_review`; `tools/validation/product_evidence_gate_v4_4.py` now requires every requested base (`006/007/008/009/015/022`) to have those artifacts, a valid application UI screenshot, manual visual judgement PASS, v6 visual acceptance PASS, and reference-compare-with-UI PASS before `requested_ref6_ui_status_pass` can pass. Current refreshed reports still block expansion: 006 has the required artifact paths but fails manual/v6/reference-with-UI, and `007/008/009/015/022` each lack all five required per-drawing artifact categories because expansion has not been allowed.
- 2026-06-26 strict 006 prune order tightened for the current over-dense dimension failure. In strict reference-intent mode, `.trae/specs/build-v6-and-validate-exe-ui/drw_generate_v6.py` now deletes generic/non-reference DisplayDims before duplicate target DisplayDims, while preserving unique target coverage. `test_v3_generator_reference_style_plan.py` proves a high-score generic AutoDimension-like survivor is removed before a duplicate target dimension, and `tools/validation/lb26001_006_rerun_packet_v4_2.py` now source-guards `generic_non_reference_intent_displaydim`, `target_rank`, and `delete_priority` evidence. No CAD, COM document open, OCR, YOLO, visual-audit batch, staged CAD validation, or batch validation was run. Refreshed `lb26001_correction_plan_v4_2.json` matches the latest 006 UI review; refreshed `lb26001_006_rerun_packet_v4_2.json` is `packet_build_ready=true`, `offline_prerequisite_missing_keys=[]`, and `real_cad_allowed_now=false` because readiness still blocks on `solidworks_unsaved_document_visible`. Refreshed `product_evidence_gate_v4_4.json` remains `status=blocked_by_solidworks_stability_gate`, `pass=false`, and does not allow 006 rerun, expansion, `LB26001_36`, `full_129`, or release.
- 2026-06-26 strict 006 generic-survivor blocker added. The same prune path now records `generic_non_reference_intent_survivor_count` / `generic_non_reference_intent_survivors` and fails with `generic_non_reference_intent_displaydim_survived_after_prune` if a targetless AutoDimension-style DisplayDim survives after the count is already at the reference-intent cap/floor. This closes the metric gap where `DisplayDim=12/12` could still leave the application UI screenshot visually AutoDimension-like. `test_v3_generator_reference_style_plan.py` covers the count-equals-cap survivor case, and `test_v4_2_lb26001_006_rerun_packet.py` requires the rerun packet to source-guard this blocker. Current refreshed diagnostics remain intentionally blocked: `lb26001_006_rerun_packet_v4_2.json` has `packet_build_ready=true` and `real_cad_allowed_now=false`; `product_evidence_gate_v4_4.json` is still `blocked_by_solidworks_stability_gate`.
- 2026-06-26 SolidWorks Stability Gate diagnostics were hardened and refreshed without running CAD, COM document operations, OCR, YOLO, visual-audit batch, staged CAD validation, or batch validation. `unguarded_solidworks_entrypoints.json` now carries explicit `pass`, `scan_root`, `total_findings`, `unguarded_count`, `missing_lock_count`, and `ui_thread_risk_count` fields; refreshed values are `pass=true`, `entrypoint_count=527`, `total_findings=527`, `unguarded_count=0`, `missing_lock_count=0`, `ui_thread_risk_count=0`, `ui_thread_subprocess_call_count=0`, `ui_thread_heavy_work_count=0`, and `service_direct_risk_count=0`. `conflict_report.json` now carries explicit `status`, `pass`, `fail_count`, and `warning_count`; refreshed values are `status=warning`, `pass=false`, `warning_count=1`, and `fail_count=0` because one `SLDWORKS.exe` is present without a worker lock. `solidworks_lock_test_result.json` remains `pass=true`. `lb26001_006_regression_readiness_v4_2.json` blocks on `solidworks_unsaved_document_visible`, the rerun packet remains `real_cad_allowed_now=false`, and Product Evidence Gate remains `blocked_by_solidworks_stability_gate`.
- 2026-06-26 006 rerun recovery instructions were aligned with the architecture gate. `tools/validation/lb26001_006_regression_readiness_v4_2.py` now emits `locked_006_cad_execution_path` showing the future single-run chain `staged_cad_validation_v3.py --stage LB26001_006 -> real_cad_smoke_v3.py -> JobRuntimeFacade.start_cad_job -> JobRunner.start_job -> QProcess cad_job_worker.py -> SolidWorks global lock`; `tools/validation/lb26001_006_rerun_packet_v4_2.py` carries the same path on the `locked_006_real_cad_rerun` gate while keeping the staged summary command needed for Drawing Review UI screenshot review. Refreshed readiness/rerun packet/Product Gate still block all CAD because `solidworks_unsaved_document_visible` is present. No CAD, COM document operation, OCR, YOLO, batch, automatic restart, or kill command was run.
- 2026-06-26 006 application UI screenshot review was hardened to require an explicit side-by-side visual layout contract. `tools/ui_robot/drawing_visual_review_suite.py` now captures the Drawing Review workbench in a direct lightweight host, records `application_ui_source_mode=drawing_review_workbench_direct_host`, `solidworks_probe_allowed_during_screenshot_review=false`, and writes `side_by_side_reference_generated_layout` with the reference PNG on the left, generated PNG on the right, both images loaded, and `api_only_acceptance_allowed=false`. `tools/validation/apply_ui_visual_review_v4.py` now blocks canonical `ui_visual_review.json` when this side-by-side proof is missing or reversed, and `tools/validation/product_evidence_gate_v4_4.py` requires `checks.side_by_side_reference_generated_layout_pass=true` before `canonical_006_ui_visual_review_pass` can pass. The earlier full-`MainWindow` screenshot runner had already produced diagnostic churn in `drw_output/diagnostics/conflict_report.json` and `drw_output/visual_audit_index.json`; after the direct-host fix, the refreshed 006 screenshot run produced `evidence_capture_pass=true`, `visual_acceptance_pass=false`, and pre/post SHA256 plus mtime checks showed both files were unchanged, proving the fixed run did not trigger System Health/SolidWorks probes or Visual Audit side effects. The closed-loop application UI result remains `status=need_review`, `pass=false`; requested drawings status remains `blocked_by_006`, `pass_count=0`, `not_pass_count=6`; Product Evidence Gate remains `status=blocked_by_solidworks_stability_gate`, `pass=false`, with no allowed expansion or release action.
- 2026-06-26 canonical 006 UI review no-probe proof was promoted into the closed-loop and Product Gate artifacts. `tools/validation/apply_ui_visual_review_v4.py` now requires and writes `application_ui_source_mode=drawing_review_workbench_direct_host`, `solidworks_probe_allowed_during_screenshot_review=false`, and `ui_screenshot_review_no_solidworks_probe_pass=true`; `tools/validation/product_evidence_gate_v4_4.py` now rejects canonical 006 UI review if that proof is missing, even when screenshot and side-by-side evidence exist. Refreshed `ui_visual_review.json` records `ui_screenshot_review_no_solidworks_probe_all_pass=true`, `side_by_side_reference_generated_layout_pass=true`, and a valid `2600x1400` screenshot, but still returns `status=need_review`, `pass=false` because UI defect bucket closure, v6 visual acceptance, and reference-compare-with-UI remain non-pass. Refreshed Product Evidence Gate remains `status=blocked_by_solidworks_stability_gate`, `pass=false`, and keeps `expand_007_008_009_015_022_allowed=false`.
- 2026-06-26 006 manual visual judgement template was hardened for bucket-level defect closure without running CAD, COM document operations, OCR, YOLO, batch validation, or screenshot recapture. `tools/validation/manual_visual_judgement_template_v4.py` now reads the 006 UI defect bucket report and injects `ui_defect_bucket_closure_checklist` into generated templates for `LB26001-A-04-006` only. Refreshed `drw_output/ui_acceptance/LB26001_006_locked_real_rerun_20260625_041353_visual_review/manual_visual_judgement_template.json` contains all six required buckets with `pass=null`, `visual_acceptance_pass=null`, `api_or_displaydim_metric_alone_can_close=false`, and the callout checklist for `thread_callout_m4_6h`, `surface_finish_rest_3_2`, `radius_callout`, and `chamfer_callout`. After refreshing `lb26001_correction_plan_v4_2.json`, `lb26001_006_rerun_packet_v4_2.json` returned to `packet_build_ready=true`, `offline_prerequisite_missing_keys=[]`, and `real_cad_allowed_now=false`; the only current packet blocker remains `solidworks_unsaved_document_visible`.
- 2026-06-26 requested six-drawing UI status now requires the same Drawing Review source contract as canonical 006. `tools/validation/lb26001_requested_drawings_status_v4_2.py` records `application_ui_source_mode`, `solidworks_probe_allowed_during_screenshot_review`, `ui_screenshot_review_no_solidworks_probe_pass`, and `side_by_side_reference_generated_layout_pass` for each requested base; `tools/validation/product_evidence_gate_v4_4.py` includes those fields in `requested_ref6_ui_status_pass`. Refreshed `lb26001_requested_drawings_status_v4_2.json` remains `status=blocked_by_006`, `pass=false`, `pass_count=0`, `not_pass_count=6`. The 006 row has direct-host/no-probe and side-by-side proof, but still fails manual visual judgement, defect bucket closure, v6 visual acceptance, and reference-compare-with-UI; `007/008/009/015/022` are explicitly missing no-probe and side-by-side UI review proof plus required per-drawing artifacts. Product Evidence Gate remains `blocked_by_solidworks_stability_gate` and does not allow requested-sample completion, `LB26001_36`, `full_129`, or release.

006 Reference Intent Dimension Plan evidence:

- `drw_output/reference_intent_dimension_plan_006.json`: regenerated with schema `sw_drawing_studio.reference_intent_dimension_plan.v4_4`; it contains 12 required DisplayDim targets and every target has `source_reference`, `target_view`, `expected_type`, `is_manufacturing_dimension`, `fallback_policy`, `source_reference_evidence`, and a visual reference value.
- Visual reference values recorded from `drw_output/case_library/LB26001-A-04-006.png`: overall length `230`, width `12`, height `13`, end offsets `10/10`, hole x stations `[10, 80, 150, 220]`, hole y station `6`, pitch `[70, 70, 70]`, `4-⌀3.3` through holes, `M4-6H` through thread, and `3.2 其余`.
...
- `drw_output/diagnostics/lb26001_006_reference_intent_proof_v4_4.json` and `.md`: new offline Task 2 proof, generated by `tools/validation/lb26001_006_reference_intent_proof_v4_4.py`. Current result is `status=plan_proof_pass_requires_locked_cad_run`, `pass=true`, `release_ready=false`, and `report_is_drawing_acceptance_evidence=false`; it proves the 12-target plan covers total length/width/height, end offsets, hole diameter/x/y/pitch, right-side projection width/height/small-feature location, M4-6H and 3.2 callouts, radius/chamfer absence policy, no Note-as-DisplayDim substitution, and CAD-worker lock-only contract execution.
- `test_v4_4_lb26001_006_reference_intent_proof.py`: verifies the current proof passes, and blocks missing right-projected-view targets, Note substitution, UI-thread/unlocked contract execution, and accidental COM/subprocess usage in the proof tool.
- `tools/validation/product_evidence_gate_v4_4.py`: now directly reads raw Task 2 artifacts `drw_output/reference_intent_dimension_plan_006.json` and `drw_output/reference_intent_dimension_contract_006.json`. New Product Gate checks `reference_intent_006_plan_complete` and `reference_intent_006_contract_locked_worker_only` require the 12 required manufacturing DisplayDim targets, required callouts, no Note-as-DisplayDim substitution, no generic AutoDimension acceptance, SolidWorks global lock execution, `ui_thread_may_execute=false`, and `direct_com_called=false`. Current Product Gate records both checks as `pass`; they remain supporting/orchestration evidence only and do not replace the fresh locked CAD run or application UI screenshot acceptance.
...
- `tools/validation/lb26001_006_regeneration_evidence_gate_v4_4.py`: new offline Task 3 fresh-regeneration evidence gate. It accepts only explicit `--run-dir` or staged `--summary` evidence, verifies `drw_output/runs/<run_id>` ownership, requires `manifest.run_id`, checks SLDDRW/PDF/DXF/PNG/QC/vision/final-quality artifact freshness against job start time, verifies `sw_session.json`, and requires `job_started/progress/heartbeat/job_finished` in `job_event_log.jsonl`. It does not call SolidWorks and it explicitly reports `report_is_drawing_acceptance_evidence=false`.
...
- `drw_output/diagnostics/product_evidence_gate_v4_4.json` and `.md`: current result is `status=blocked_by_solidworks_readiness`, `pass=false`, `release_ready=false`, with blockers `solidworks_readiness_for_006`, `regeneration_006_fresh_evidence_pass`, `application_ui_006_acceptance_pass`, `requested_ref6_ui_status_pass`, `final_release_artifacts_present`, `exe_ui_and_stability_proof_pass`, and `visual_audit_schema_proof_pass`. The report sets `locked_006_cad_rerun_allowed_now=false`, `expand_007_008_009_015_022_allowed=false`, `requested_ref6_complete=false`, `lb26001_36_allowed=false`, `full_129_allowed=false`, and `release_allowed=false`.
- `tools/validation/product_evidence_gate_v4_4.py`: allowed-action semantics are now stricter. Any real-CAD follow-on action remains blocked when current readiness is blocked; `expand_007_008_009_015_022_allowed` means 006 UI PASS plus current readiness allows starting the remaining samples, while `requested_ref6_complete` separately proves all six requested samples already passed application UI screenshot review.
- `tools/validation/product_evidence_gate_v4_4.py`: now includes `visual_audit_schema_proof_pass`. The gate requires final `drw_output/visual_audit_report_v3_0.xlsx`, raw `drw_output/issue_schema_validation.json` with `pass=true` and `noncompliant_issue_count=0`, normalized `drw_output/issue_schema_validation_normalized.json` with `pass=true`, and the v4.4 Visual Audit schema gap diagnostic with `pass=true`; normalized proof is explicitly supporting-only and cannot replace raw historical issue compliance.
- `drw_output/diagnostics/product_evidence_gate_v4_4.json` and `.md`: current blockers now also include `visual_audit_schema_proof_pass`, because `visual_audit_report_v3_0.xlsx` is missing and raw issue schema validation remains `pass=false` while normalized issue schema validation is only supporting evidence.
- `tools/validation/visual_audit_schema_gap_v4_4.py`: new file-only Visual Audit schema gap diagnostic. It aggregates raw issue schema validation, normalized issue schema validation, Product Evidence Gate allowed actions, `visual_audit_index.json`, and final `visual_audit_report_v3_0.xlsx` presence without calling SolidWorks or launching workers. Current `drw_output/diagnostics/visual_audit_schema_gap_v4_4.json` is `status=raw_issue_schema_noncompliant`, `pass=false`, `raw_noncompliant_issue_count=4330`, `normalized_noncompliant_issue_count=0`, `visual_audit_report_final_present=false`, `visual_audit_full_scope_allowed_now=false`, and `blocking_issue_keys=["raw_issue_schema_pass","final_visual_audit_report_present","visual_audit_full_scope_allowed"]`. This makes the normalized report explicitly supporting-only and not a replacement for raw historical issue compliance.
- `tools/validation/product_evidence_gate_v4_4.py`: now directly reads `drw_output/diagnostics/visual_audit_schema_gap_v4_4.json` and includes the gap status in `visual_audit_schema_proof_pass` details. Refreshed `drw_output/diagnostics/product_evidence_gate_v4_4.json` has `source_artifacts.visual_audit_schema_gap` set to the diagnostic path and reports the gap as `status=raw_issue_schema_noncompliant`, `pass=false`, `raw_noncompliant_issue_count=4330`, `normalized_supporting_only=true`, and `normalized_cannot_replace_raw=true`.
- `tools/validation/product_evidence_gate_v4_4.py`: now directly reads `drw_output/diagnostics/lb26001_006_rerun_packet_v4_2.json` before allowing any 006 locked CAD rerun or downstream `LB26001_36` / `medium_30` / Visual Audit full scope / `full_129` / release action. The Product Gate requires `packet_build_ready=true`, no `offline_prerequisite_missing_keys`, `report_is_acceptance_evidence=false`, `api_only_acceptance_allowed=false`, `application_ui_screenshot_is_final_gate=true`, and a rerun-packet readiness state matching the current readiness report. Current `drw_output/diagnostics/product_evidence_gate_v4_4.json` records `lb26001_006_rerun_packet_ready=pass` and `lb26001_006_rerun_packet_readiness_state_current=pass`; the packet itself remains `status=blocked_by_solidworks_readiness`, `real_cad_allowed_now=false`, and `offline_prerequisite_missing_keys=[]`.
- `tools/validation/product_evidence_gate_v4_4.py`: now directly reads canonical `drw_output/ui_acceptance/LB26001_006_locked_real_rerun_20260625_041353_visual_review/closed_loop/ui_visual_review.json` for the 006 application screenshot gate. The new `canonical_006_ui_visual_review_pass` check requires `review_method=application_drawing_review_ui_screenshot`, `application_ui_screenshot_is_final_gate=true`, `api_only_acceptance_allowed=false`, a passing `LB26001-A-04-006` entry, `visual_acceptance_pass=true`, and a nonempty screenshot file. Current Product Gate records this check as `fail` because the canonical UI review is `status=need_review`, `pass=false`, `fail_count=1`; the screenshot file exists, but the visual judgement remains non-pass.
- `tools/validation/product_evidence_gate_v4_4.py`: now includes `exe_ui_and_stability_proof_pass`. The gate requires `dist/sw_drawing_studio.exe`, canonical `exe_ui_robot_result_v3_0.json` with EXE/windows PASS semantics, canonical `stability_20min_mock_v3_0.json` with PASS and observed duration >= 1200s, and canonical `stability_2h_ui_v3_0.json` with Windows/EXE PASS and observed duration >= 7200s.
- `tools/validation/product_evidence_gate_v4_4.py`: final artifact coverage now follows `docs/final_acceptance_criteria.md` more closely. In addition to EXE/stability and Visual Audit artifacts, `final_release_artifacts_present` now checks canonical `validation_log_v3_0.md`, `ui_acceptance_report_v3_0.md`, `cad_smoke_v3_0.json`, `dimension_validation_smoke.json`, `reference_compare_smoke.json`, `drw_output/reference_comparison_report_v3_0.xlsx`, and `drw_output/visual_audit_index.json`.
- `test_v4_4_product_evidence_gate.py`: verifies a complete synthetic evidence fixture can pass, readiness blocks all real-CAD follow-on actions, 006 UI acceptance failure blocks expansion, 006 UI PASS can allow only the ref6 expansion start while keeping `LB26001_36/full_129` blocked until all six pass, missing final artifacts block release, raw issue schema failure blocks release even when normalized schema passes, EXE stability duration shortfall blocks release, source-level UI robot evidence cannot masquerade as EXE evidence, and the gate remains file-only.
...
python -B -m py_compile tools\validation\apply_ui_visual_review_v4.py tools\validation\lb26001_006_rerun_packet_v4_2.py tools\validation\product_evidence_gate_v4_4.py test_v4_apply_ui_visual_review.py test_v4_2_lb26001_006_rerun_packet.py test_v4_4_product_evidence_gate.py
python -B test_v4_apply_ui_visual_review.py
python -B test_v4_2_lb26001_006_rerun_packet.py
...
python test_v4_2_lb26001_006_rerun_packet.py
python tools\validation\lb26001_006_rerun_packet_v4_2.py --out-json drw_output\diagnostics\lb26001_006_rerun_packet_v4_2.json --out-md drw_output\diagnostics\lb26001_006_rerun_packet_v4_2.md
python -m py_compile tools\validation\lb26001_006_reference_intent_proof_v4_4.py test_v4_4_lb26001_006_reference_intent_proof.py test_v4_4_reference_intent_plan_006.py test_v4_2_reference_intent_dimension_planner.py
python test_v4_4_lb26001_006_reference_intent_proof.py
python tools\validation\lb26001_006_reference_intent_proof_v4_4.py --out-json drw_output\diagnostics\lb26001_006_reference_intent_proof_v4_4.json --out-md drw_output\diagnostics\lb26001_006_reference_intent_proof_v4_4.md
python tools\validation\run_solidworks_stability_gate_v4_4.py
...
python -B -m py_compile tools\validation\lb26001_006_regeneration_evidence_gate_v4_4.py test_v4_4_lb26001_006_regeneration_evidence_gate.py app\workers\cad_job_worker.py
...
python -B test_v4_2_lb26001_006_rerun_packet.py
python -B test_v4_apply_ui_visual_review.py
python -B -m py_compile tools\validation\visual_audit_schema_gap_v4_4.py test_v4_4_visual_audit_schema_gap.py
...
python -B test_v4_2_lb26001_006_rerun_packet.py
python -B test_v4_4_product_evidence_gate.py
python -B tools\validation\lb26001_correction_plan_v4_2.py
python -B tools\validation\lb26001_006_rerun_packet_v4_2.py --out-json drw_output\diagnostics\lb26001_006_rerun_packet_v4_2.json --out-md drw_output\diagnostics\lb26001_006_rerun_packet_v4_2.md
python -B tools\validation\product_evidence_gate_v4_4.py --out-json drw_output\diagnostics\product_evidence_gate_v4_4.json --out-md drw_output\diagnostics\product_evidence_gate_v4_4.md
python -B test_v4_4_lb26001_006_regeneration_evidence_gate.py
python -B tools\validation\lb26001_006_rerun_packet_v4_2.py --out-json drw_output\diagnostics\lb26001_006_rerun_packet_v4_2.json --out-md drw_output\diagnostics\lb26001_006_rerun_packet_v4_2.md
...
Result: PASS for SolidWorks Stability Gate, bottom-level entrypoint/lock/conflict evidence, raw 006 reference-intent plan/contract verification, 006 reference-intent proof, 006 UI defect-bucket classification, rerun-packet regression, canonical UI visual-review regression, diagnostics worker smoke, source-level UI robot quick suite, fresh-regeneration evidence-gate unit tests, Product Evidence Gate unit tests, Visual Audit schema gap unit tests, and regression tests. The standalone regeneration evidence report intentionally returns a blocked result (`status=blocked_by_missing_fresh_006_run`) because SolidWorks is currently not running and no fresh locked 006 run was launched. The Visual Audit schema gap report intentionally returns `status=raw_issue_schema_noncompliant`, `pass=false`, and `release_ready=false` because raw historical issue schema compliance is not proven, the final Visual Audit report is missing, and full-scope Visual Audit is not allowed yet. The Product Evidence Gate intentionally returns `status=blocked_by_solidworks_readiness`, `pass=false`, and `release_ready=false`; it now also blocks release on `exe_ui_and_stability_proof_pass`, `visual_audit_schema_proof_pass`, and `canonical_006_ui_visual_review_pass` because canonical EXE/stability artifacts, final Visual Audit report, raw issue schema compliance, and the canonical 006 UI visual review are not proven. It also refuses 006/downstream real-CAD actions unless the raw reference-intent plan/contract, 006 rerun packet, current readiness state, and raw entrypoint/lock/conflict reports remain green. Overall v4.4 remains WARNING because 006 still requires a fresh locked CAD worker run plus application Drawing Review UI screenshot acceptance.
...
- Status remains `WARNING / NOT RELEASE READY`.
- This is an offline source and artifact hardening step for the `titlebar_incomplete` application UI screenshot bucket.
- No real CAD, COM, `OpenDoc6`, `SaveAs`, `CloseDoc`, OCR, YOLO, batch validation, Visual Audit full scope, automatic restart, or release action was run.
- `LB26001-A-04-006` is still not accepted, and `007/008/009/015/022` remain blocked until 006 passes the application Drawing Review UI screenshot review.
...
- Updated `app/services/reference_intent_dimension_planner.py`.
  - 006 now emits `reference_titlebar_policy` and `sheet_template_policy`.
  - The policy records that the reference screenshot has no default lower-right title block.
  - Required values include:
    ...
    - bottom notice text `细节尺寸较多，未注尺寸请核对3D图档`
- Updated `.trae/specs/build-v6-and-validate-exe-ui/drw_generate_v6.py`.
  - Carries the titlebar policy from `reference_intent_dimension_plan_006.json` into the blueprint layout plan.
  - `_v4_blueprint_titlebar_insertions()` now suppresses `图号/品名` visible titlebar notes when the 006 reference policy requires it.
  - The generator inserts only the reference-style bottom notice for this policy.
- Updated `tools/validation/lb26001_006_reference_intent_proof_v4_4.py`.
  - Reference proof now requires the titlebar suppression policy and bottom notice evidence.
- Updated `tools/validation/product_evidence_gate_v4_4.py`.
  - Product Gate now requires the current 006 plan to prove default titlebar suppression.
- Updated `tools/validation/lb26001_006_rerun_packet_v4_2.py`.
  - Source signatures now guard `ui_defect_bucket_suppress_default_titlebar_fields` and `DrawingBlueprint.reference_titlebar_policy`.
- Updated tests:
  - `test_v3_generator_reference_style_plan.py`
  - `test_v4_2_reference_intent_dimension_planner.py`
  - `test_v4_4_product_evidence_gate.py`
...
```powershell
python -B -m py_compile app\services\reference_intent_dimension_planner.py .trae\specs\build-v6-and-validate-exe-ui\drw_generate_v6.py tools\validation\lb26001_006_rerun_packet_v4_2.py tools\validation\product_evidence_gate_v4_4.py tools\validation\lb26001_006_reference_intent_proof_v4_4.py test_v3_generator_reference_style_plan.py test_v4_2_reference_intent_dimension_planner.py test_v4_4_product_evidence_gate.py test_v4_4_lb26001_006_reference_intent_proof.py
python -B test_v4_2_reference_intent_dimension_planner.py
python -B test_v3_generator_reference_style_plan.py
python -B test_v4_4_lb26001_006_reference_intent_proof.py
python -B test_v4_4_reference_intent_plan_006.py
...
- `test_v4_1_solidworks_entrypoint_scan.py`: PASS.
- `test_v4_4_product_evidence_gate.py`: PASS.
- Refreshed 006 plan contains `suppress_default_titlebar_fields=true` and `render_reference_bottom_notice=true`.
- Refreshed reference proof is `plan_proof_pass_requires_locked_cad_run`, `pass=true`.
- Refreshed 006 readiness remains blocked:
  - `status=blocked`
  - `blocking_issue_keys=["solidworks_unsaved_document_visible"]`
...
- Updated `tools/validation/product_evidence_gate_v4_4.py`.
  - Adds `solidworks_readiness_title_sampling_guard`.
  - Requires readiness JSON to include no-COM multi-sample SolidWorks title evidence:
    - `solidworks_process.sample_count >= 5`
    - at least one process observation when SolidWorks is present
    - `solidworks_process.unsaved_title_observed=false`
  - Blocks old readiness JSON files that only report `status=ready` / `ready_to_start_locked_006_cad=true` without title sampling evidence.
  - Blocks contradictory readiness JSON where `status=ready` but an unsaved `*` title was observed.
  - `locked_006_cad_rerun_allowed_now` now requires both `solidworks_readiness_for_006` and `solidworks_readiness_title_sampling_guard`.
- Updated `test_v4_4_product_evidence_gate.py`.
  - Fixture now emits the readiness sampling schema.
  - Adds regression coverage for old readiness JSON without title sampling.
  - Adds regression coverage for ready-state JSON with `unsaved_title_observed=true`.
...
- Compile check: PASS.
- `test_v4_4_product_evidence_gate.py`: PASS.
- Refreshed 006 readiness remains blocked correctly:
  - `status=blocked`
  - `ready_to_start_locked_006_cad=false`
  - `blocking_issue_keys=["solidworks_unsaved_document_visible"]`
  - `sample_count=5`
...
- Refreshed 006 rerun packet remains `blocked_by_solidworks_readiness`, `packet_build_ready=true`, `offline_prerequisite_missing_keys=[]`, and `real_cad_allowed_now=false`.
- Refreshed SolidWorks Stability Gate remains `warning`, with source scan and lock tests passing, because the current idle SolidWorks process is not under a worker-owned global lock.
- Refreshed Product Gate remains `blocked_by_solidworks_stability_gate` and now explicitly includes `solidworks_readiness_title_sampling_guard` in `blocking_issue_keys`.
- All follow-on actions remain false:
  - `locked_006_cad_rerun_allowed_now=false`
  - `006_application_ui_review_allowed_now=false`
  - `expand_007_008_009_015_022_allowed=false`
